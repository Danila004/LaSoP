...
//Реализовать консольное приложение для управления компанией, состоящей
//из множества отделов — добавление, удаление, изменение, просмотр отдела;
//управления выбранным отделом — добавление, редактирование, удаление
//сотрудников; выборка по критерию(см.задачу 2) по отделу с возможностями печати
//и перемещения сотрудников в другой отдел.
//Добавить «формулу полезности» сотрудника, которая переводит все его поля
//в некоторое числовое значение(на свой вкус).Добавить возможность удалять
//самого бесполезного сотрудника из отдела и возможность удалять самого
//бесполезного сотрудника из компании.
//Предусмотреть вывод статистики по всей компании : таблицу вида «название
//отдела — количество сотрудников — суммарный оклад, наибольший и наименьший
//оклады cсотрудников в отделе», а ниже строку «наибольший и наименьший оклады
//сотрудников во всей компании», информацию о лучшем сотруднике.
...
short menu()
{
	...
	std::cout << "7.Удалить худшего сотрудника отдела\n";
	std::cout << "8.Завершить работу с отделом\n\n->";
	short choice;
	std::cin >> choice;
	if (std::cin.fail() || choice <=> 1 == std::strong_ordering::less || choice <=> 8 == std::strong_ordering::greater) throw std::invalid_argument("Невалидный аргумент");
	return choice;
}
...
{
	std::cout << "\nВыберете задачу:\n";
	std::cout << "1.Добавить отдел\n";
	std::cout << "2.Удалить отдел\n";
	std::cout << "3.Поработать с отделом\n";
	std::cout << "4.Вывести статистику\n";
	std::cout << "5.Удалить самого худшего сотрудника\n";
	std::cout << "6.Напечатать\n";
...
{
	std::cout << "\nВыберете задачу для выборки:\n";
	std::cout << "1.Напечатать\n";
	std::cout << "2.Перенести в другой отдел\n";
	short choice;
	std::cin >> choice;
	if (std::cin.fail() || choice <=> 1 == std::strong_ordering::less || choice <=> 2 == std::strong_ordering::greater) throw std::invalid_argument("Невалидный аргумент");
	return choice;
			...
			case 4:
			{
				if (!x.get()[i].empty())
				{
					...
					else
					{
						...
						if (index <=> i == 0) std::cout << "\nЭти работники уже в выбранном отделе\n";
						else
						{
							x.remove(index, tmp);
...
int find_index(Company& x, std::string name)
{
	...
	throw std::invalid_argument("Нет такого отдела");
}

std::string input_department_name()
{
	std::cout << "\nВведите название отдела, который хотите удалить:\n\n->";
	std::string name{};
	std::cin >> name;
	return name;
}
...
void input_company(Company& x, bool& exit)
{
	...
	{
		example_employee();
		std::cout << "\nВведите компанию, разделяя сотрудников из разных отделов строкой вида ======================. Перед каждым отделом введите его название. В конце ввода компании тоже нужна строка ================:\n";
		std::istream_iterator<Department> in(std::cin);
		x.input(in);
		std::cin.clear();
		std::cin.ignore(std::cin.rdbuf()->in_avail());
			...
			if (!x.empty())
			{
				...
				case 2:
				{
					if (submenu_for_del_or_change() <=> 1 == 0)
						...
						x.del(input_index(x));
					std::cout << "\nУдаление проведено успешно, если такой отдел есть в компании\n\n";
					break;
				}
				case 3:
				{
